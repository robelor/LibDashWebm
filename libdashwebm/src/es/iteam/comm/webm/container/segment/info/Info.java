package es.iteam.comm.webm.container.segment.info;
	...
	public static Info create(Element infoElement, EBMLReader ebmlReader, DataSource dataSource) {
		...
		while (auxElement != null) {
			...
			if (auxElement.equals(MatroskaDocType.TimecodeScale_Id)) {
				...
				if (D)
					Log.d(LOG_TAG, WebmContainer.class.getSimpleName() + ": " + "      TimeCodeScale: " + timeCodeScale);
				...
				if (D)
					Log.d(LOG_TAG, WebmContainer.class.getSimpleName() + ": " + "      Duration: " + duration);
				info.setDuration(duration);
			} else if (auxElement.equals(MatroskaDocType.Title_Id)) {
				String title = ((StringElement) auxElement).getValue();
				if (D)
					Log.d(LOG_TAG, WebmContainer.class.getSimpleName() + ": " + "      Title: " + title);
				info.setTitle(title);
			}
